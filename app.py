...
with st.sidebar:
    ...
    if bucket_type == t["buckets_default"]:
        ...
        # Instruction hint
        hint_pl = "Wprowadź przedziały w dniach. Przykład: 0–90 oznacza 0–3 miesiące."
        hint_en = "Enter aging ranges in days. Example: 0–90 means 0–3 months."
        st.markdown(
            f'<div class="cb-hint">{hint_pl if lang == "pl" else hint_en}</div>',
            unsafe_allow_html=True
        )
        ...
        label_from = t.get("bucket_from", "Od dnia" if lang == "pl" else "From day")
        label_to   = t.get("bucket_to",   "Do dnia" if lang == "pl" else "To day")
        label_name = t.get("bucket_name", "Nazwa" if lang == "pl" else "Name")
        label_rem  = t.get("bucket_remove", "Usuń" if lang == "pl" else "Remove")
        ...
        for i, b in enumerate(cb_list):
            ...
            st.markdown(
                f'<div class="cb-card">'
                f'<div class="cb-card-header">#{i+1}</div>'
                f'</div>',
                unsafe_allow_html=True
            )

            # Use a container so CSS .cb-inputs scoping works
            ...
            with col_t:
                ...
                if is_last:
                    ...
                    new_to = int(to_val) if int(to_val) > 0 else None
                else:
                    to_val = st.number_input(
                        label_to,
            ...
            # Range label display
            range_str = f"{int(new_from)} – {'∞' if new_to is None else int(new_to)}"
            st.markdown(
                f'<div style="font-size:0.70rem;color:rgba(255,255,255,0.55);'
                f'margin:-2px 0 4px 0;">{range_str} dni / days</div>',
                unsafe_allow_html=True
            )

            # Remove button (scoped red style via .cb-remove)
            st.markdown('<div class="cb-remove">', unsafe_allow_html=True)
            if st.button(label_rem, key=f"cb_rem_{i}"):
                st.session_state["_remove_bucket_idx"] = i
                st.rerun()
            st.markdown('</div>', unsafe_allow_html=True)

            # Update bucket in session state
            cb_list[i] = {"name": new_name, "from_day": int(new_from), "to_day": new_to}

        st.session_state["custom_buckets"] = cb_list

        # Add bucket button (ghost style via .cb-add)
        ...
        if bucket_warnings:
            for w in bucket_warnings:
                st.markdown(
                    ...
                    f'padding:5px 8px;font-size:0.72rem;color:#FFF3CD;margin-top:3px;">⚠️ {w}</div>',
                    unsafe_allow_html=True
                )
        if bucket_errors:
            for e in bucket_errors:
                st.markdown(
                    ...
                    f'padding:5px 8px;font-size:0.72rem;color:#FADADD;margin-top:3px;">❌ {e}</div>',
                    unsafe_allow_html=True
                )
            buckets = default_buckets_list   # fallback to defaults on hard errors
        else:
            buckets = cb_list
