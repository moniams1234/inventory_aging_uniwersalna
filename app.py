...
with st.sidebar:
    ...
    if bucket_type == t["buckets_default"]:
        ...
        for i, b in enumerate(cb_list):
            ...
            st.markdown(f'<div class="cb-card"><div class="cb-card-header custom-bucket-index">#{i+1}</div></div>', unsafe_allow_html=True)
            ...
            st.markdown(
                f'<div class="custom-bucket-help">{rstr} dni / days</div>',
                unsafe_allow_html=True
            )
